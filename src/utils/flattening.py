...
def flatten_study(study):
    """Extract a flat dictionary of useful trial info
    from a nested JSON record
    """
    ...
    status = (
        study.get("protocolSection", {}).get("statusModule", {}).get("overallStatus")
    )
    enrollment = (
    ...
    return {
        ...
        "conditions": (
            ", ".join(condition) if isinstance(condition, list) else condition
        ),
        "sponsor": sponsor,
    }
